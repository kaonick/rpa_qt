...
def call_azure_openai_api(prompt: str, api_key: str, model: str = "gpt-4") -> str:
    """Call OpenAI API and return response"""
    if not HAS_OPENAI:
        raise ImportError("openai package not installed. Run: pip install openai")
    ...
    api_version = os.getenv("AZURE_API_VERSION")  # Use the latest API version
    ...
    # Remember to use your specific model deployment name
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "You are a professional portfolio analyst. Always respond with valid JSON in the exact format requested."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.3,
            max_tokens=1500
...
def run_automated_trading(api_key: str, model: str = "gpt-4", data_dir: str = "Start Your Own", dry_run: bool = False):
    ...
    response = call_azure_openai_api(prompt, api_key, model)
...
def main():
    ...
    args.api_key = os.getenv("AZURE_OPENAI_API_KEY")
    args.model = os.getenv("DEPLOYMENT_NAME")

    args.data_dir = "case_01"
